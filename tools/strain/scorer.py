...
def jensen_shannon_divergence(tokens_a: list[str], tokens_b: list[str]) -> float:
    ...
    # Normalize to [0, 1] (JSD is already bounded by log2(2) = 1)
    return min(jsd, 1.0)
...
def components_to_score(components: StrainComponents) -> float:
    ...
    strain = 1.0 - similarity + 0.2 * components.bigram_divergence
...
def extract_claiming_context(
    ...
    if idx >= 0:
        ...
        # The claiming context is the FULL sentence (including the citation)
