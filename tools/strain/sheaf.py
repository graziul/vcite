...
DESIGN.md S3. Given a set of local strain measurements, checks
...
def cluster_by_source(
    ...
    if citations and len(citations) == len(local_strains):
        ...
        if citations and len(citations) != len(local_strains):
            print(
                f"WARNING: citations ({len(citations)}) and local_strains "
                f"({len(local_strains)}) have different lengths — "
                f"falling back to per-citation grouping. "
                f"Sheaf analysis will report trivial consistency.",
                file=sys.stderr,
            )
